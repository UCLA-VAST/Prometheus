...
int main(int argc, char* argv[]) {
    ...
    cl::Buffer A_oriNewOCL =
        cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                   sizeof(float) * 390*410, A_ori, &err);
    if (err != CL_SUCCESS) {
    ...
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects(
                       {x_oriNewOCL, A_oriNewOCL}, 0, nullptr, nullptr));
    q.finish();
    cl::Event kernelCompute;
    OCL_CHECK(err, err = q.enqueueTask(kernel, nullptr, &kernelCompute));
    q.finish();
    kernelCompute.wait();
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({y_oriNewOCL,tmp_oriNewOCL},
