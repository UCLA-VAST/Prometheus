...
int main(int argc, char* argv[]) {
    ...
    cl::Buffer buf4NewOCL =
        cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                   sizeof(float) * 180*220, output, &err);
    if (err != CL_SUCCESS) {
    ...
    q.finish();
    kernelCompute.wait();
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buf4NewOCL},
