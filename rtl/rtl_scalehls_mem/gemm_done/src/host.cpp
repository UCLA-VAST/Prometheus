...
int main(int argc, char* argv[]) {
    ...
    kernel.setArg(argN++, v3);
    kernel.setArg(argN++, v4);
    kernel.setArg(argN++, v5NewOCL);

    
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects(
                       {v0NewOCL, v1NewOCL}, 0, nullptr, nullptr));
    q.finish();
    cl::Event kernelCompute;
    OCL_CHECK(err, err = q.enqueueTask(kernel, nullptr, &kernelCompute));
    q.finish();
    kernelCompute.wait();
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({v2NewOCL,v5NewOCL},
