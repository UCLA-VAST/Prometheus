...
int main(int argc, char* argv[]) {
    ...
    kernel.setArg(argN++, v5NewOCL);
    kernel.setArg(argN++, v6NewOCL);
    
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects(
                       {v0NewOCL, v1NewOCL, v2NewOCL, v3NewOCL}, 0, nullptr, nullptr));
    q.finish();
    cl::Event kernelCompute;
    OCL_CHECK(err, err = q.enqueueTask(kernel, nullptr, &kernelCompute));
    q.finish();
    kernelCompute.wait();
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({v4NewOCL, v5NewOCL, v6NewOCL},
