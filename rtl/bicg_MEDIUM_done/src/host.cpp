...
int main(int argc, char *argv[]) {
  ...
  float A_new_before_trans_0[413 * 400] = {0};
  float A_new_0[413 * 400] = {0};
  float A_new_before_trans_1[413 * 400] = {0};
  float A_new_1[413 * 400] = {0};
  float s_ori[390] = {0};
  ...
  cl::Buffer A_0NewOCL =
      cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
                 sizeof(float) * 413 * 400, A_new_0, &err);
  if (err != CL_SUCCESS) {
  ...
  cl::Buffer A_1NewOCL =
      cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
                 sizeof(float) * 413 * 400, A_new_1, &err);
  if (err != CL_SUCCESS) {
    std::cerr << "Could not allocate buffer ANewOCL, error number: " << err
              << "\n";
    return EXIT_FAILURE;
