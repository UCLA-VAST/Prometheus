...
#pragma HLS inline off
  float tmp_0[400];
  // FT0
#pragma HLS array_partition variable = tmp_0 cyclic factor = 400 dim = 1
  float tmp_1[400];
  // FT0
#pragma HLS array_partition variable = tmp_1 cyclic factor = 400 dim = 1
  float A[400][416];
#pragma HLS array_partition variable = A cyclic factor = 400 dim = 1
...
#pragma HLS array_partition variable = x cyclic factor = 2 dim = 1
  ...
  write_tmp_FT0(tmp_0, fifo_tmp_from_task1_to_task3, 0);
  write_tmp_FT0(tmp_0, fifo_tmp_to_off_chip, 0);
}
...
#pragma HLS array_partition variable = y_1 cyclic factor = 16 dim = 1
  float A_0[400][16];
  // FT1
#pragma HLS array_partition variable = A_0 cyclic factor = 50 dim = 1
#pragma HLS array_partition variable = A_0 cyclic factor = 16 dim = 2
  float A_1[400][16];
  // FT1
#pragma HLS array_partition variable = A_1 cyclic factor = 50 dim = 1
...
#pragma HLS inline off
  for (int d0_1 = 0; d0_1 < 400; d0_1 += 16) {
    ...
    if (d0 + 8 < 400)
      tmp[d0 + 0 + 8] = tmp_fifo[8];
    if (d0 + 9 < 400)
      tmp[d0 + 0 + 9] = tmp_fifo[9];
    if (d0 + 10 < 400)
      tmp[d0 + 0 + 10] = tmp_fifo[10];
    if (d0 + 11 < 400)
      tmp[d0 + 0 + 11] = tmp_fifo[11];
    if (d0 + 12 < 400)
      tmp[d0 + 0 + 12] = tmp_fifo[12];
    if (d0 + 13 < 400)
      tmp[d0 + 0 + 13] = tmp_fifo[13];
    if (d0 + 14 < 400)
      tmp[d0 + 0 + 14] = tmp_fifo[14];
    if (d0 + 15 < 400)
      tmp[d0 + 0 + 15] = tmp_fifo[15];
  }
}
void read_tmp_FT1(
    float tmp[400],
...
#pragma HLS inline off
  for (int d0_1 = 0; d0_1 < 400; d0_1 += 16) {
    ...
    if (d0 + 8 < 400) {
      uint32_t tmp_8 = tmp_fifo.data.range(287, 256);
      tmp[d0 + 0 + 8] = *(float *)(&tmp_8);
    }
    if (d0 + 9 < 400) {
      uint32_t tmp_9 = tmp_fifo.data.range(319, 288);
      tmp[d0 + 0 + 9] = *(float *)(&tmp_9);
    }
    if (d0 + 10 < 400) {
      uint32_t tmp_10 = tmp_fifo.data.range(351, 320);
      tmp[d0 + 0 + 10] = *(float *)(&tmp_10);
    }
    if (d0 + 11 < 400) {
      uint32_t tmp_11 = tmp_fifo.data.range(383, 352);
      tmp[d0 + 0 + 11] = *(float *)(&tmp_11);
    }
    if (d0 + 12 < 400) {
      uint32_t tmp_12 = tmp_fifo.data.range(415, 384);
      tmp[d0 + 0 + 12] = *(float *)(&tmp_12);
    }
    if (d0 + 13 < 400) {
      uint32_t tmp_13 = tmp_fifo.data.range(447, 416);
      tmp[d0 + 0 + 13] = *(float *)(&tmp_13);
    }
    if (d0 + 14 < 400) {
      uint32_t tmp_14 = tmp_fifo.data.range(479, 448);
      tmp[d0 + 0 + 14] = *(float *)(&tmp_14);
    }
    if (d0 + 15 < 400) {
      uint32_t tmp_15 = tmp_fifo.data.range(511, 480);
      tmp[d0 + 0 + 15] = *(float *)(&tmp_15);
    }
  }
}
...
#pragma HLS pipeline II = 1
    ...
    tmp_fifo[8] = tmp[d0 + 0 + 8];
    tmp_fifo[9] = tmp[d0 + 0 + 9];
    tmp_fifo[10] = tmp[d0 + 0 + 10];
    tmp_fifo[11] = tmp[d0 + 0 + 11];
    tmp_fifo[12] = tmp[d0 + 0 + 12];
    tmp_fifo[13] = tmp[d0 + 0 + 13];
    tmp_fifo[14] = tmp[d0 + 0 + 14];
    tmp_fifo[15] = tmp[d0 + 0 + 15];
