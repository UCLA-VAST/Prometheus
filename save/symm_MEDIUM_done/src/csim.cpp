...
void kernel_nlp(float alpha, float beta, float8 vtmp_for_task1[6000],
...
int main() {
  ...
  float A1_new_before_trans_0[200 * 200] = {0};
  float A1_new_0[200 * 200] = {0};
  float B1_ori[200][240] = {0};
  float B1_new_before_trans_0[200 * 240] = {0};
  float B1_new_0[200 * 240] = {0};
  float A2_ori[200] = {0};
  float A2_new_before_trans_0[208] = {0};
  float A2_new_0[208] = {0};
  float B2_ori[200][240] = {0};
  float B2_new_before_trans_0[200 * 240] = {0};
  float B2_new_0[200 * 240] = {0};
  float A3_ori[200][200] = {0};
  float A3_new_before_trans_0[200 * 200] = {0};
  float A3_new_0[200 * 200] = {0};
  ...
  for (int i0 = 0; i0 < 200; i0++) {
    for (int i1 = 0; i1 < 200; i1++) {
      val = ((float)rand() / RAND_MAX);
      A1_ori[i0][i1] = val;
      A1_new_before_trans_0[i0 * 200 + i1 * 1] = val;
      A1_new_0[i0 * 200 + i1 * 1] = val;
    }
  }
  // if padding we need to change data order
  memIndex = 0;
  ...
  for (int d0_0 = 0; d0_0 < 25; d0_0++) {
    for (int d0_1 = 0; d0_1 < 8; d0_1++) {
      for (int d1 = 0; d1 < 200; d1++) {
        int d0 = d0_0 * 8 + d0_1;
        A1_new_0[memIndex] = A1_new_before_trans_0[d0 * 200 + d1 * 1];
        memIndex++;
      }
    }
  }
  ...
  for (int i0 = 0; i0 < 200; i0++) {
    for (int i1 = 0; i1 < 200; i1++) {
      val = ((float)rand() / RAND_MAX);
      A3_ori[i0][i1] = val;
      A3_new_before_trans_0[i0 * 200 + i1 * 1] = val;
      A3_new_0[i0 * 200 + i1 * 1] = val;
    }
  }
  // if padding we need to change data order
  memIndex = 0;
    ...
    for (int d0_0 = 0; d0_0 < 25; d0_0++) {
      for (int d0_1 = 0; d0_1 < 8; d0_1++) {
        for (int d1_1 = 0; d1_1 < 8; d1_1++) {
          int d0 = d0_0 * 8 + d0_1;
          int d1 = d1_0 * 8 + d1_1;
          A3_new_0[memIndex] = A3_new_before_trans_0[d0 * 200 + d1 * 1];
          memIndex++;
        }
      }
    }
  ...
  kernel_nlp(alpha_new, beta_new, (float8 *)tmp_new_0, (float16 *)B1_new_0,
             (float8 *)A1_new_0, (float8 *)C_new_0, (float16 *)B2_new_0,
             (float16 *)A2_new_0, (float16 *)B3_new_0, (float8 *)A3_new_0);
  memIndex = 0;
  for (int d0_0 = 0; d0_0 < 25; d0_0++) {
    for (int d1_0 = 0; d1_0 < 10; d1_0++) {
      for (int d0_1 = 0; d0_1 < 8; d0_1++) {
